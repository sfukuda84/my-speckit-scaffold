...
argument-hint: "フィーチャー番号または範囲と、任意の --auto（例: 001, 002-005, all, all --auto, または省略して次の未着手）"
...
引数の解釈と複数フィーチャーの進め方は `speckit-worktree` の §5 に従う。`--auto` があるときは、下の 💬 の質問も含めて `speckit-worktree` §6 の自動モードで進める。自動検出では `--phase spec` を使う。
