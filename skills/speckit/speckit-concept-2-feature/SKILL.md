...
| 条件 | モード | 手順 |
|---|---|---|
| `docs/feature/` がない、または機能ファイルも `premises.md` もない（`.gitkeep` だけなど） | **新規モード** | §2 |
| `docs/feature/` に機能ファイルか `premises.md` があり、`--backlog` なし | **再実行モード** | §2 を §5 の規則で行う |
| `docs/feature/` に機能ファイルか `premises.md` があり、`--backlog` あり | **バックログモード** | §6 |
| 新規モードの条件に当てはまり、`--backlog` あり | 新規モードで進めてよいかを質問する | — |
...
- 画面・サーバー側の処理（API、Server Actions など）・スキーマ・認証・DB アクセスを読み、機能ごとに **実装済み / 一部実装 / 未実装** と、concept との差を `docs/concept/implementation-status.md` に書く（ファイルパスを添える）。技術基盤と共通標準（architecture 文書）との差も書く。
...
- **共通基盤が提供するものは機能にしない**（認証方式、セッション、キュー、ストレージ、ロギング、運営者向けの管理機能など）。各機能の「前提・設計原則との関係」から参照する。
...
| `人の作業待ち（specs/NNN-slug）` | AI のタスクは完了し、`tasks.md` に人が行うタスク（`[人]`）だけが残っている |
| `完了` | spec に沿った実装が完了した |
...
- concept のファイルを、`competitor.md` への追記、`backlog.md` と `implementation-status.md` の作成・更新以外で書き換えること
